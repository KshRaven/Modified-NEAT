...
class Grid:
    ...
    def base_array_discrete(self) -> np.ndarray:
        """(rows, cols, 2) int32: [wall_bitmask (0-15, bit0=UP,bit1=RIGHT,bit2=DOWN,bit3=LEFT),
        cell_type_code (0=EMPTY,1=START,2=GOAL,3=TRAP)]. Small non-negative integers,
        directly usable as nn.Embedding indices."""
        arr = np.zeros((self.rows, self.cols, 2), dtype=NP_INT)
        for r in range(self.rows):
            for c in range(self.cols):
                cell = self.cells[r, c]
                bitmask = 0
