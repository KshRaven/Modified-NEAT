...
class Game(Env):
    ...
    def __init__(
        ...
        **params: dict[str, Any],
        ...
        self._build_spaces(1)

    # ------------------------------------------------------------------
    def _build_spaces(self, total: int) -> None:
        state = self.agents.get_state()
        if self.agents.discrete_states:
            high = max(self.grid.rows, self.grid.cols, 16) + 1
            self.observation_space = spaces.Box(low=0, high=high, shape=state.shape, dtype=np.int64)
        else:
            self.observation_space = spaces.Box(low=-1.0, high=1.0, shape=state.shape, dtype=np.float32)

        if self.agents.discrete_actions:
            self.action_space = spaces.MultiDiscrete([4] * total)
        else:
            self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(total, 2), dtype=np.float32)
