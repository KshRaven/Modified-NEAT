...
from ModifiedNEAT.rl.base import Algorithm
...
class NEAT(Algorithm):
    def __init__(self, model: Model, population: Population, device=torch.device('cpu'), dtype=torch.float32, **options):
        ...
        :keyword log_name:
        """
        schedulers = manage_params(options, 'schedulers', None)
        super().__init__(model, population, schedulers)
    ...
    def _train(self, batches: dict[int, list[list[int]]], states: TensorDict, actions: TensorDict,
        ...
        # Compute advantages
        advantages = self._get_advantages(batches, states, rewards) if self.loss_reg != 0 else rewards
        # Create dictionaries
        pl, vl, el, tl, cf, kd = {}, {}, {}, {}, {}, {}
        ...
        for key in self.replay.mapping.keys():
            if key in batches and self.loss_reg != 0:
                ...
                pl[key] = vl[key] = el[key] = tl[key] = cf[key] = kd[key] = 0 # np.nan
